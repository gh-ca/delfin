...
class SSHHandler(object):
    ...
    HPE3PAR_COMMAND_SHOWFPG = 'showfpg'
    HPE3PAR_COMMAND_SHOWCPG = 'showcpg'
    HPE3PAR_COMMAND_SHOWVV = 'showvv'
    HPE3PAR_COMMAND_SHOWFSTORE = 'showfstore'
    HPE3PAR_COMMAND_SHOWFSHARE = 'showfshare'
    HPE3PAR_COMMAND_SHOWVFS = 'showvfs'
    ...
    def get_fpgs(self):
        return self.get_resources_info(SSHHandler.HPE3PAR_COMMAND_SHOWFPG,
                                       self.analyse_datas_to_list,
                                       pattern_str=consts.FPG_PATTERN)

    def get_cpg_id_map(self):
        para_map = {
            'key_position': 1,
            'value_position': 0
        }
        return self.get_resources_info(SSHHandler.HPE3PAR_COMMAND_SHOWCPG,
                                       self.analyse_datas_to_map,
                                       pattern_str=consts.CPG_PATTERN,
                                       para_map=para_map, ckeck_excep=False)

    def get_volumes_map(self):
        volume_map = {}
        volumes = self.get_resources_info(
            SSHHandler.HPE3PAR_COMMAND_SHOWVV,
            self.analyse_datas_to_list, pattern_str=consts.VOLUME_PATTERN,
            ckeck_excep=False)
        if volumes:
            for volume in volumes:
                volume_map[volume.get('name')] = volume
        return volume_map

    def get_fstores(self):
        para_map = {
            'command': 'showfstore'
        }
        return self.get_resources_info(SSHHandler.HPE3PAR_COMMAND_SHOWFSTORE,
                                       self.analyse_datas_to_list,
                                       pattern_str=consts.FSTORE_PATTERN,
                                       para_map=para_map)

    def get_fshares(self):
        return self.get_resources_info(SSHHandler.HPE3PAR_COMMAND_SHOWFSHARE,
                                       self.analyse_datas_to_list,
                                       pattern_str=consts.FSHARE_PATTERN)

    def get_fpg_of_vfs(self):
        para_map = {
            'key_position': 0,
            'value_position': 1
        }
        return self.get_resources_info(SSHHandler.HPE3PAR_COMMAND_SHOWVFS,
                                       self.analyse_datas_to_map,
                                       pattern_str=consts.VFS_PATTERN,
                                       para_map=para_map, ckeck_excep=False)
            ...
            for obj_info in obj_infos:
                ...
                if str_line:
                    ...
                    if search_obj:
                        ...
                        if para_map:
                            if para_map.get('command', '') == 'showfstore':
                                titles.pop()
                                titles_size = len(titles)
                    else:
    ...
    def analyse_disk_inventory(self, cols_size, titles_size, str_info,
        ...
        if cols_size == titles_size:
            inventory_map = {
                'disk_id': str_info[0],
                'disk_mfr': ' '.join(str_info[4:cols_size - 6]),
                'disk_model': str_info[cols_size - 6],
                'disk_serial': str_info[cols_size - 5],
                'disk_fw_rev': str_info[cols_size - 4]
            }
