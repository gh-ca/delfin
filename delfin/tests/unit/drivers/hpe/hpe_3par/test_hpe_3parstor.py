...
testFpg0 /testFpg0 1024.00 1023.32 ACTIVATED cpg1 testFpg0.1 degraded 11.0*
testFpg1 /testFpg1 1024.00 1023.32 ACTIVATED SSD_r6 testFpg1.1 normal 11.1
------------------------------------------------------------------------------
2 total 2048.00 2046.64
"""
CPG_DATAS = """
              ----Volumes---- -Usage- ----------(MiB)-----------
Id Name Warn% VVs TPVVs TDVVs Usr Snp Base Snp Free Total
2 cpg1 - 2 0 0 2 1 24576 512 36352 60928
0 FC_r1 - 2 2 0 2 2 1024 1024 30720 32768
1 SSD_r6 - 2 0 2 2 1 1536 512 34816 36864
-----------------------------------------------------------------
3 total 6 4 27136 103936 101888 130560
"""
VOLUME_DATAS = """
                    -Rsvd(MiB)-    -(MiB)--
Id Name Prov Compr Dedup Type CopyOf BsId  Rd  -Detailed_State- Snp Usr VSize
3  testFpg0.1  dds  NA  No  base  ---  3  RW  normal  0  512  67108864
2   testFpg1.1  tdvv  No  Yes  base  ---  2  RW  normal  0  512  17408
4   dedup.2  tdvv  No  Yes  base  ---  4  RW  normal  0  512  17408
------------------------------------------------------------------------------
7  total                  0  183296  67342336
"""
FSTORE_DATAS = """
---------------Security-------
Fstore VFS FPG State Mode ErrorSuppression -Comment-
.admin vfs1 fpg1 normal legacy false
fstore1 vfs1 fpg1 normal legacy false
----------------------------------
4 total
"""
FSHARE_DATAS = """
ShareName Protocol VFS FileStore ShareDir State
smb1 smb vfs1 fstore1 ./ normal
nfs1 nfs vfs2 fstore2 ./ normal
-------------------------------------------------
2 total
"""
VFS_DATAS = """
VFS FPG IPAddr State -Comment-
vfs1 fpg1 10.13.25.53 normal -
vfs2 fpg2 10.13.25.53 normal -
---------------------------------------
1 total
"""
...
FS_RESULT = [
    {
        ...
        'used_capacity': 730144441,
        'free_capacity': 1098781483335
    }]
FSTORE_RESULT = [
    {
        'name': '.admin',
        'storage_id': '12345',
        'native_qtree_id': 'fpg1/vfs1/.admin',
        'native_filesystem_id': 'fpg1',
        'path': None,
...
FSHARE_RESULT = [
    {
        'name': 'smb1',
        'storage_id': '12345',
        'native_share_id': 'smb/vfs1/fstore1/smb1',
        'native_filesystem_id': 'fpg1',
        'native_qtree_id': 'fstore1',
        'path': './',
        'protocol': 'cifs'
    }]
...
class TestHpe3parStorageDriver(TestCase):
    ...
    def test_get_fss(self):
        driver = create_driver()
        SSHHandler.do_exec = mock.Mock(
            side_effect=[FPG_DATAS, CPG_DATAS, VOLUME_DATAS])
        filesystems = driver.list_filesystems(context)
        self.assertDictEqual(filesystems[0], FS_RESULT[0])

    def test_get_qtrees(self):
        driver = create_driver()
        SSHHandler.do_exec = mock.Mock(side_effect=[FSTORE_DATAS])
        qtrees = driver.list_qtrees(context)
        self.assertDictEqual(qtrees[0], FSTORE_RESULT[0])

    def test_get_shares(self):
        driver = create_driver()
        SSHHandler.do_exec = mock.Mock(side_effect=[FSHARE_DATAS, VFS_DATAS])
        shares = driver.list_shares(context)
        self.assertDictEqual(shares[0], FSHARE_RESULT[0])
